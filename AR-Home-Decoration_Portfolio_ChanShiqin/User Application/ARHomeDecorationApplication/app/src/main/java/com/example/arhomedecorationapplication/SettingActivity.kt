...
class SettingActivity : AppCompatActivity() {
    ...
    private lateinit var productImageRef: DatabaseReference
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val moreOrderButton: TextView = findViewById(R.id.viewMoreOrderText)
        moreOrderButton.setOnClickListener {
        ...
        if (!userId.isNullOrEmpty()) {
            userRef.child(userId).addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        userEmail.text = snapshot.child("email").getValue(String::class.java) ?: "No Email"
                        userName.text = snapshot.child("name").getValue(String::class.java) ?: "No Name"
                        ...
                        if (!profilePictureUrl.isNullOrEmpty()) {
                            ...
                            if (decodedBitmap != null) {
                                userProfilePicture.setImageBitmap(decodedBitmap)
                            } else {
                                userProfilePicture.setImageResource(R.drawable.baseline_person_24) // Default image placeholder
                            }
                        } else {
                            userProfilePicture.setImageResource(R.drawable.baseline_person_24) // Default image placeholder
                        }
    ...
    private fun fetchUserOrders() {
        ...
        if (!userId.isNullOrEmpty()) {
            orderRef.orderByChild("userId").equalTo(userId).addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
...
//                orderView.findViewById<TextView>(R.id.productName).text = "$productName"
        ...
        viewDetails.setOnClickListener {
            val intent = Intent(this@SettingActivity, OrderHistoryProductDetailsActivity::class.java)
    ...
    private fun logout() {
        ...
        val logoutButton: LinearLayout = findViewById(R.id.logoutLayout)
    ...
    private fun footerActivity() {
        ...
        val viewMoreShareableFolderText: TextView = findViewById(R.id.viewMoreShareableFolderText)
        viewMoreShareableFolderText.setOnClickListener {
